...
import { DEFAULT_SETTINGS, USE_MULTI_LANGUAGE } from '../../../config.jsx';
...
const MainMenuItem = memo(({ item, id, isSubItem = false, menuPlacement = DEFAULT_SETTINGS.MENU_PLACEMENT }) => {
  ...
  return (
    <Dropdown.Item as="li" id={item.label}>
      <a href={item.path} className={classNames({ active: isActive })} activeClassName="">
        {getLabel(item.icon, item.label)}
      </a>
    </Dropdown.Item>
  );
